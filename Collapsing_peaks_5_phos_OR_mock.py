...
if len(sys.argv)<2:
    ...
    OutFileName1=OutFileNameList[0]+'_marked_for_collapse.txt'
    OutFileName2=OutFileNameList[0]+'_marked_for_collapse_min5reads_mock_or_phos.txt'
    OutFileName3=OutFileNameList[0]+'_collpased_min5reads_mock_or_phos.txt'
    ...
    #make a new column that indicates whether a coord has the highest coverage in a 9-nt window in mean phos library coverage
    myDF1['highest_in_9'] = ((
                                   (myDF1.mean_phos_coverage > np.roll(myDF1.mean_phos_coverage,-1)) &
                                    (myDF1.mean_phos_coverage > np.roll(myDF1.mean_phos_coverage,-2)) &
                                    (myDF1.mean_phos_coverage > np.roll(myDF1.mean_phos_coverage,-3)) &
                                    (myDF1.mean_phos_coverage > np.roll(myDF1.mean_phos_coverage,-4)) &
                                    (myDF1.mean_phos_coverage > np.roll(myDF1.mean_phos_coverage,+1)) &
                                    (myDF1.mean_phos_coverage > np.roll(myDF1.mean_phos_coverage,+2)) &
                                    (myDF1.mean_phos_coverage > np.roll(myDF1.mean_phos_coverage,+3)) &
                                    (myDF1.mean_phos_coverage > np.roll(myDF1.mean_phos_coverage,+4))))
    ...
    collapsed_DF = pd.DataFrame(myDF2[(myDF2.highest_in_9 == True)])
